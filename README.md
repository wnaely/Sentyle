...
## 1. 개요
   ...
   * ### 데이터 및 모델 개요
     ...
     데이터는 2022년 쇼핑몰과 SNS 한국어 리뷰 데이터로 구성되어 있고 패션, 화장품, 가전, IT기기, 생활 분야로 나눠져 있다.
     리뷰와 라벨링 데이터는 텍스트와 JSON 형태로 저장되어 있으며, 이중에서 패션 분야의 쇼핑몰 리뷰 데이터로 프로젝트를 진행하려 한다.
...
## 2. 데이터
   ...
   * ### 원본 데이터 소개
    ...
    "GeneralPolarity": "1" <br>
       ...
       이 라벨링된 데이터에서 상품평 리뷰 긍부정 라벨인 "GeneralPolarity" 데이터를 기존 원본 데이터에 추가하였다. 1이 긍정, 0이 중립, -1은 부정으로 라벨링 되어 있으며, 라벨링이 되어있지 않은 애매한 리뷰는 no general polarity로 분류하였다.

     결측치와 중복값을 제거하고 글자수가 20 미만인 리뷰는 삭제하여 데이터 전처리를 진행한다.
